...
const BLOG_CONTRACT = `local json = require('json')
...
export async function deployBlogInBrowser(options: {
  ...
  try {
    ...
    const completeContract = `${ACCESS_CONTROL_CONTRACT}\n\n${contractWithRegistry}`;

    // Deploy the contract using aoconnect
    const result = await aoconnect.spawn({
      module: completeContract,
      scheduler: aosConfig.scheduler,
      signer: signer,
      tags: [
    ...
    try {
      ...
      console.warn(
        '⚠️ Failed to sync initial permissions with registry:',
        syncError
      );
